...
def main(page: ft.Page):
    ...
    macro_row = ft.Row([ft.Text("B: 0"), ft.Text("T: 0"), ft.Text("W: 0")], alignment="center")
    ...
    def refresh_data():
        ...
        kcal_txt.value = f"{int(k)} / {int(target)} kcal"; pb.value = min(k / target, 1.0) if target > 0 else 0
        macro_row.controls[0].value = f"B: {int(b)}g"; macro_row.controls[0].color = "red"
        macro_row.controls[1].value = f"T: {int(t)}g"; macro_row.controls[1].color = "orange"
        macro_row.controls[2].value = f"W: {int(w)}g"; macro_row.controls[2].color = "blue"
    ...
    def delete_item(e):
        conn = get_db(); cur = conn.cursor(); cur.execute("DELETE FROM dziennik WHERE id=%s", (e.control.data,)); conn.commit(); conn.close(); refresh_data()

    def add_meal(e):
        if not prod_dd.value or not amt_in.value: return
        conn = get_db(); cur = conn.cursor(); cur.execute("SELECT kcal, b, t, w, jednostka FROM produkty WHERE nazwa=%s", (prod_dd.value,))
        p = cur.fetchone(); v = float(amt_in.value.replace(",", ".")); m = v / 100 if p[4] == '100g' else v
        cur.execute("INSERT INTO dziennik (user_name, data, posilek, nazwa, ilosc, jednostka, kcal, b, t, w) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)", (st["u"], st["d"].strftime("%Y-%m-%d"), meal_dd.value, prod_dd.value, v, p[4], p[0]*m, p[1]*m, p[2]*m, p[3]*m))
        conn.commit(); conn.close(); amt_in.value = ""; refresh_data()

    def save_prod(e):
        conn = get_db(); cur = conn.cursor(); cur.execute("INSERT INTO produkty VALUES (%s,%s,%s,%s,%s,%s) ON CONFLICT (nazwa) DO UPDATE SET kcal=EXCLUDED.kcal, b=EXCLUDED.b, t=EXCLUDED.t, w=EXCLUDED.w, jednostka=EXCLUDED.jednostka", (n_nazwa.value, float(n_k.value or 0), float(n_b.value or 0), float(n_t.value or 0), float(n_w.value or 0), n_typ.value))
        conn.commit(); conn.close(); n_nazwa.value=""; n_k.value=""; n_b.value=""; n_t.value=""; n_w.value=""; load_prods(); page.open(ft.SnackBar(ft.Text("Dodano!")))

    def save_cel(e):
        conn = get_db(); cur = conn.cursor(); cur.execute("UPDATE profile SET cel_kcal=%s WHERE user_name=%s", (float(p_cel.value), st["u"])); conn.commit(); conn.close(); p_cel.value=""; refresh_data()

    def save_waga(e):
        conn = get_db(); cur = conn.cursor(); cur.execute("INSERT INTO waga_log VALUES (%s,%s,%s) ON CONFLICT (user_name, data) DO UPDATE SET waga=EXCLUDED.waga", (st["u"], st["d"].strftime("%Y-%m-%d"), float(p_waga.value.replace(",",".")))); conn.commit(); conn.close(); p_waga.value=""; refresh_data()
    ...
    tab_baza = ft.Column([ft.Text("NOWY PRODUKT", size=20, weight="bold"), ft.Row([n_nazwa, n_typ]), ft.Row([n_k, n_b, n_t, n_w]), ft.ElevatedButton("DODAJ DO BAZY", on_click=save_prod, bgcolor="green", color="white")], visible=False)

    # NAPRAWIONA LINIA WYLOGUJ (bez argumentu color)
    tab_profil = ft.Column([
        ...
        waga_list, ft.TextButton("WYLOGUJ", on_click=lambda _: page.window_reload())
    ...
    page.navigation_bar = ft.NavigationBar(destinations=[ft.NavigationBarDestination(icon=ft.Icons.BOOK, label="Dziennik"), ft.NavigationBarDestination(icon=ft.Icons.ADD_CIRCLE, label="Baza"), ft.NavigationBarDestination(icon=ft.Icons.PERSON, label="Profil")], on_change=nav_change, visible=False)
